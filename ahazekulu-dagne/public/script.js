...
const firebaseConfig = {
    apiKey: "AIzaSyBSikYn-ReajKp7zj6z0ZQJ4kd4fs9MzEo",
    authDomain: "ahaze-6abd4.firebaseapp.com",
    projectId: "ahaze-6abd4",
    storageBucket: "ahaze-6abd4.firebasestorage.app",
    messagingSenderId: "255914762739",
    appId: "1:255914762739:web:eb8cbbd2749ec3737e3286",
    measurementId: "G-DPHEFXT11T"
  };
...
(async function() {
    ...
    // Configuration
    cloudinary.config({ 
        cloud_name: 'dnvng9ped', 
        api_key: '535555375864111', 
...
// Add this after the Firebase initialization (after `const auth = getAuth(app);` and Cloudinary config)
async function initializeFirestoreData() {
    try {
        // Places collection (Ethiopian hierarchy)
        ...
        if (placesSnapshot.empty) {
            const initialPlaces = [
                { 
                    ...
                    type: 'country', 
                    parentId: null, 
                    childCounts: { regions: 2, zones: 0, woredas: 0, kebeles: 0 } 
                },
                { 
                    id: 'oromia', 
                    name: 'Oromia', 
                    type: 'region', 
                    parentId: 'ethiopia', 
                    childCounts: { zones: 1, woredas: 0, kebeles: 0 } 
                },
                { 
                    id: 'jimma', 
                    name: 'Jimma', 
            ...
            for (const place of initialPlaces) {
                await setDoc(doc(placesCollection, place.id), place);
            }
            console.log('Initialized places in Firestore');
        }
        ...
        if (postsSnapshot.empty) {
            ...
            console.log('Initialized posts in Firestore');
        }
        ...
        if (usersSnapshot.empty) {
            await setDoc(doc(usersCollection, 'user1'), {
                ...
                address: { country: 'ethiopia', region: 'oromia', zone: 'jimma' },
                profile: {}
            });
            console.log('Initialized users in Firestore');
        }
        ...
        if (agentsSnapshot.empty) {
            const initialAgents = [
                ...
                { id: 'agent2', placeId: 'tigray', name: 'Agent Tadesse', isLive: true }
            ];
            for (const agent of initialAgents) {
                await setDoc(doc(agentsCollection, agent.id), agent);
            }
            console.log('Initialized agents in Firestore');
        }

        // Latest updates collection
        const latestCollection = collection(db, 'latest');
        ...
        if (latestSnapshot.empty) {
            const initialLatest = [
                { 
                    id: 'latest1', 
                    placeId: 'ethiopia', 
                    content: 'Peace & Security: Stable in Ethiopia.', 
                    category: 'peace', 
                    timestamp: new Date().toISOString() 
                }
            ];
            for (const item of initialLatest) {
                await setDoc(doc(latestCollection, item.id), item);
            }
            console.log('Initialized latest updates in Firestore');
        }
    } catch (error) {
...
// Update loadPlaces to use Firestore
async function loadPlaces() {
    try {
        ...
        places = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
            childCounts: doc.data().childCounts || { regions: 0, zones: 0, woredas: 0, kebeles: 0 }
        }));
        window.placesLoaded = true;
        window.placesData = places;
        console.log('Loaded places from Firestore:', places.length);
        return places;
    } catch (error) {
        console.error('Error loading places from Firestore:', error.message);
        return [];
    }
}
...
// Replace the existing handlePostSubmit function (around line 1000-1050 in your original script.js)
async function handlePostSubmit(e) {
    ...
    try {
        let mediaUrl = null, videoUrl = null, fileUrl = null;
        if (mediaFiles.length > 0) {
            const file = mediaFiles[0];
            const formData = new FormData();
            formData.append('file', file);
            formData.append('upload_preset', 'your-upload-preset'); // Set in Cloudinary Dashboard
            ...
            if (file.type.startsWith('image/')) mediaUrl = data.secure_url;
            else if (file.type.startsWith('video/')) videoUrl = data.secure_url;
            else if (file.type === 'application/pdf') fileUrl = data.secure_url;
        }
        ...
        const newPost = {
            ...
            content: text,
            user: currentUser ? users[currentUser].identifier : 'Anonymous',
            timestamp: new Date().toISOString(),
            mediaUrl,
            videoUrl,
            fileUrl
        ...
        const postsCollection = collection(db, 'posts');
        await setDoc(doc(postsCollection, newPost.id), newPost);
...
// Replace the existing document.addEventListener('DOMContentLoaded', ...) in script.js
document.addEventListener('DOMContentLoaded', async function() {
    console.log('DOM fully loaded, initializing...');
    try {
        // Initialize Firestore data if empty and load places
        await initializeFirestoreData(); // Ensure this function is defined above
        await loadPlaces(); // Ensure this function is defined above
        ...
        // Robust resetUI function for navigation
        function resetUI() {
            document.querySelectorAll('.nav-list a').forEach(a => a.classList.remove('nav-active'));
            const homeContents = document.querySelectorAll('.home-content');
            homeContents.forEach(content => content.classList.add('hidden')); // Hide all matching elements
            if (homeContents.length === 0) console.warn('No .home-content elements found in DOM');
            document.getElementById('place-title').style.display = 'none';
            document.getElementById('place-tabs').style.display = 'none';
            document.querySelectorAll('.tab-content').forEach(content => content.style.display = 'none');
            const authForms = document.querySelector('.auth-forms');
            if (authForms) authForms.classList.remove('show');
        }
        ...
        if (homeLink) {
            homeLink.onclick = (e) => {
                ...
                const homeContents = document.querySelectorAll('.home-content');
                homeContents.forEach(content => content.classList.remove('hidden'));
                if (homeContents.length === 0) console.warn('No .home-content to show for Home');
            };
            homeLink.click(); // Initial load
        }

        const exploreLink = document.querySelector('.nav-list a[data-path="/explore"]');
        if (exploreLink) {
            exploreLink.onclick = (e) => {
                e.preventDefault();
                resetUI();
                exploreLink.classList.add('nav-active');
                showPlaceContent('ethiopia');
            };
        }

        const agentsLink = document.querySelector('.nav-list a[data-path="/agents"]');
        if (agentsLink) {
            agentsLink.onclick = (e) => {
                e.preventDefault();
                resetUI();
                agentsLink.classList.add('nav-active');
                const selectedPlaceId = document.querySelector('.nav-list a.active')?.dataset.placeId || 'ethiopia';
                buildSidenav(window.places, 'agents', selectedPlaceId);
                showTab('agents', selectedPlaceId);
                document.getElementById('place-title').style.display = 'block';
                document.getElementById('place-tabs').style.display = 'block';
            };
        ...
        const latestLink = document.querySelector('.nav-list a[data-path="/latest"]');
        if (latestLink) {
            latestLink.onclick = (e) => {
                e.preventDefault();
                resetUI();
                latestLink.classList.add('nav-active');
                const selectedPlaceId = document.querySelector('.nav-list a.active')?.dataset.placeId || 'ethiopia';
                buildSidenav(window.places, 'latest', selectedPlaceId);
                showTab('latest', selectedPlaceId);
                document.getElementById('place-title').style.display = 'block';
                document.getElementById('place-tabs').style.display = 'block';
            };
        ...
        const profileLink = document.querySelector('.nav-list a[data-path="/profile"]');
        if (profileLink) {
            profileLink.onclick = (e) => {
                e.preventDefault();
                resetUI();
                profileLink.classList.add('nav-active');
                showProfile();
                initializeProfileTabs();
            };
        }
        ...
        document.getElementById('signUpForm')?.addEventListener('submit', async (e) => {
            ...
            try {
                const userCredential = await handleSignUp(identifier, password, { country, region, zone, woreda, kebele, otherLevel });
                currentUser = userCredential.uid;
                console.log('User Signed Up:', users[currentUser]);
                alert('Sign Up successful! (Simulated)');
                document.querySelector('.auth-forms').classList.remove('show');
                document.getElementById('signUpForm').reset();
                showProfile();
            } catch (error) {
        ...
        document.getElementById('loginForm')?.addEventListener('submit', async (e) => {
            ...
            try {
                const userCredential = await handleLogin(identifier, password);
                currentUser = userCredential.uid;
                console.log('User Logged In:', users[currentUser]);
                alert('Login successful! (Simulated)');
                document.querySelector('.auth-forms').classList.remove('show');
                document.getElementById('loginForm').reset();
                showProfile();
        ...
        document.getElementById('know-form')?.addEventListener('submit', handleKnowledgeSubmit);
        ...
        document.querySelectorAll('.feature-link').forEach(link => {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                const path = link.getAttribute('data-path');
                const navLink = document.querySelector(`.nav-list a[data-path="${path}"]`);
                if (navLink) {
                    navLink.click();
                }
            });
